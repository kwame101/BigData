...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register_admin($identity, $password, $email, $additional_data = array(), $groups = array())
	{
	 ...
	 * @author Mathew
	 **/
	public function register_others($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		if ($this->identity_check($identity))
		{
	...
	/*
	* create a user account - update user section email provided
	*
	* @return bool
	* @author Kwame
	*/
	public function create_account($identity, $password, $email, $additional_data = array())
	{
		$manual_activation = $this->config->item('manual_activation', 'ion_auth');
		// IP Address
		$ip_address = $this->_prepare_ip($this->input->ip_address());
	...
	/*
	* check if email is already added into the user table
	*
